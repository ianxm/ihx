...
class TestBuiltins extends haxe.unit.TestCase
{
    ...
    {
        var proc = new CmdProcessor();
        var ret = proc.process("a=new Date(2008, 1, 2, 3, 4, 5)");
    ...
    public function testDateTools()
    {
        ...
        ret = proc.process("a=new Date(2008, 1, 2, 3, 4, 5)");
        assertEquals("2008-02-02 03:04:05", ret);
    ...
    {
        var proc = new CmdProcessor();
        var ret = proc.process("re=new EReg('word(.)','')");
        assertEquals("{ r => #abstract, global => false }", ret);
    ...
    {
        var proc = new CmdProcessor();
        var ret = proc.process("re=new EReg('word(.)','i')");
        assertEquals("{ r => #abstract, global => false }", ret);
        ret = proc.process("re.match('WORD7')");
        assertEquals("true", ret);
        ret = proc.process("re.matched(1)");
    ...
    {
        var proc = new CmdProcessor();
        var ret = proc.process("re = ~/word(.)/i");
        assertEquals("{ r => #abstract, global => false }", ret);
        ret = proc.process("re.match('WORD7')");
        assertEquals("true", ret);
        ret = proc.process("re.matched(1)");
    ...
    {
        var proc = new CmdProcessor();
        var ret = proc.process("a=new Hash()");
        assertEquals("{}", ret);
        ret = proc.process("a.set('one', 1);a");
        assertEquals("{one => 1}", ret);
        ret = proc.process("a.set('two', 2);a");
    ...
    public function testLambda()
    {
        ...
        ret = proc.process("Lambda.has(a,1,null)");
        assertEquals("true", ret);
    }
    ...
    {
        var proc = new CmdProcessor();
        var ret = proc.process("a=new List()");
        assertEquals("{}", ret);
        ret = proc.process("a.add(1);a");
        assertEquals("{1}", ret);
        ret = proc.process("a.add(4);a");
    ...
    {
        var proc = new CmdProcessor();
        var ret = proc.process("a=new StringBuf()");
        assertEquals("", ret);
        ret = proc.process("a.add('one');a");
        assertEquals("one", ret);
        ret = proc.process("a.add(' two');a");
    ...
    {
        var proc = new CmdProcessor();
        var ret = proc.process("a=Xml.parse('<one att=\"1\">word</one>')");
    ...
    {
        var proc = new CmdProcessor();
        var ret = proc.process("a=haxe.Int32.ofInt(32)");
        assertEquals("32", ret);
        ret = proc.process("b=haxe.Int32.ofInt(33)");
        assertEquals("33", ret);
    ...
    {
        var proc = new CmdProcessor();
        var ret = proc.process("a=haxe.Md5.encode('a string')");
    ...
    {
        var proc = new CmdProcessor();
        var ret = proc.process("a=haxe.Serializer.run('a string')");
    ...
    {
        var proc = new CmdProcessor();
        var ret = proc.process("a=haxe.Unserializer.run('y10:a%20string')");
        assertEquals("a string", ret);
    }
}
